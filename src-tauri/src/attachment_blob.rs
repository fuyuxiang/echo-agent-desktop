...
//! Mime-to-extension mapping covers the formats `isImageAttachment` accepts on
//! the JS side; anything unknown still lands as `.png` so the multimodal
//! pipeline has a stable extension to inspect.

use std::path::{Path, PathBuf};
use std::time::{SystemTime, UNIX_EPOCH};

/// Image MIME types the frontend will paste. Anything else falls back to `.png`
/// (the most common screenshot carrier).
fn extension_for_mime(mime: &str) -> &'static str {
    match mime.to_ascii_lowercase().as_str() {
        ...
        "image/bmp" => "bmp",
        "image/svg+xml" => "svg",
        // 未知 image/* 一律当作 PNG；非 image/* 在调用方已被过滤。
        _ => "png",
    }
}
...
fn unique_suffix() -> String {
    ...
    let rand = (pid.rotate_left(7) ^ nanos as u32 ^ (nanos >> 32) as u32) & 0xFFFF_FFFF;
...
pub fn save_blob(
    ...
    std::fs::canonicalize(&destination)
        .map_err(|error| format!("解析附件路径失败：{error}"))
}
...
mod tests {
    ...
    #[test]
    fn unknown_image_mime_falls_back_to_png_extension() {
        let temp = tempfile::tempdir().expect("temp dir");
        let path = save_blob(temp.path(), b"x", "image/avif", None).unwrap();
    ...
    fn suggested_name_strips_path_separators_and_traversal() {
        ...
        // 必须仍在 temp 目录内(不能逃逸)。
        assert!(path.starts_with(temp.path().canonicalize().unwrap()), "{path:?}");
    }
    ...
    fn empty_or_whitespace_suggested_name_falls_back_to_image() {
        ...
        for bad in [None, Some(""), Some("   "), Some("....."), Some("\t\n")] {
            ...
            let file_name = path.file_name().unwrap().to_string_lossy().to_string();
            assert!(file_name.starts_with("image-"), "got {file_name} for {bad:?}");
        }
    }
    ...
    fn long_suggested_name_is_truncated_to_80_chars() {
        ...
        assert!(file_name.len() < 200, "got len {}: {file_name}", file_name.len());
        let stem = file_name.split('-').next().unwrap();
        assert_eq!(stem.len(), 80, "stem must be exactly 80 chars, got {}", stem.len());
    }
}