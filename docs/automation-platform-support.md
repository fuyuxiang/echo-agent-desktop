...
- **电脑操作的硬安全约束**：无论任务级权限模式如何（审批 / 自动 / 始终允许），`computer_click` / `computer_drag` / `computer_type` / `computer_key` 四个工具在执行前都必须经用户独立确认。AI 在真实桌面上的副作用不可逆（购买、删除、确认对话框、退出应用），与任务级权限的"自动/始终允许"是两套独立的安全机制。该约束由后端 `automation::mod::ALWAYS_CONFIRM_TOOLS` 集中维护，属于不可被任务或组织策略放宽的安全不变量。
