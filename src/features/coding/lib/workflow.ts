...
6. 按依赖顺序逐节点执行：可测试的行为变更先写能正确失败的回归测试（本轮只写测试），然后结束本轮，让工作台运行并记录真实 RED；收到继续指令后再做最小实现、重构并运行 GREEN。不要在 RED 检查点之前写生产实现。纯文档、纯测试维护或无法合理构造 RED 的任务说明具体原因，由用户在交付报告中记录豁免。验证通过后才把节点标记为 completed；不在节点范围外“顺便”重构。
