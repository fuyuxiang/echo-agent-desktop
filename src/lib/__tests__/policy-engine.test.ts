...
describe("mergeRules", () => {
  ...
  it("同优先级后者覆盖", () => {
    const rules: PolicyRule[] = [
      ...
      { type: "permission-mode", value: "auto" },
    ];
    const set = mergeRules(rules);
    expect(getPolicyValue(set, "permission-mode")).toBe("auto");
  });
  it("不同 type 各保留一条", () => {
    const rules: PolicyRule[] = [
...
describe("策略评估器", () => {
  ...
  it("getLockedPermissionMode", () => {
    expect(getLockedPermissionMode(set)).toBe("auto");
...
describe("serialize/deserialize", () => {
  it("往返", () => {
    const set = mergeRules([{ type: "permission-mode", value: "ask" }]);
    const json = serializePolicySet(set);
    const back = deserializePolicySet(json);
    expect(back.rules).toHaveLength(1);
    expect(getPolicyValue(back, "permission-mode")).toBe("ask");
  });
  it("非法 JSON → fallback", () => {
