 ...
 *
 * EchoAgent 的企业策略(enterprise-policy)用于集中管控:技能上传策略、模型白名单、
 * 沙箱规则下发、权限模式锁定等,绑定腾讯 IOA 内网部署。EchoAgent 是个人桌面应用,
...
export function getLockedPermissionMode(set: PolicySet): "ask" | "auto" | "always-approve" | undefined {
  return getPolicyValue<"ask" | "auto" | "always-approve">(set, "permission-mode");
}
