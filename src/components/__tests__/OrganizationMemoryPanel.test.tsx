...
const api = vi.hoisted(() => ({
  ...
  orgLogin: vi.fn(),
  orgLogout: vi.fn(),
  orgMemoryPromotionsMine: vi.fn(),
  orgListScopes: vi.fn(),
  orgListDocuments: vi.fn(),
  orgListMemories: vi.fn(),
  orgListSkills: vi.fn(),
  ...
  orgSubmitMemoryCandidate: vi.fn(),
...
describe("OrganizationMemoryPanel", () => {
  ...
  it("切换 Scope 时只显示当前范围的文档", async () => {
    ...
    expect(screen.getByText("Agent 组织上下文已启用")).toBeInTheDocument();
  ...
  it("仅有个人范围时静默停用 Agent 组织上下文", async () => {
    api.orgSession.mockResolvedValue({
      loggedIn: true,
    ...
    expect(screen.queryByText("Agent 组织上下文已启用")).not.toBeInTheDocument();
  });
  ...
  it("将项目经验提交到当前范围的审核队列", async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText("写成可直接指导下一次任务的明确结论或步骤"), {
      target: { value: "发布前先验证回滚脚本" },
    });
    fireEvent.change(screen.getByPlaceholderText("背景、适用条件或决策依据（建议填写）"), {
      target: { value: "避免数据迁移失败时无法恢复" },
    });
    fireEvent.change(screen.getByPlaceholderText("例如：构建时间下降 30%"), {
      target: { value: "恢复时间缩短" },
    });
    const workspaceInput = screen.getByPlaceholderText("可选，用于项目定向召回");
    expect(workspaceInput).toHaveValue("/workspace/payments");
    fireEvent.change(workspaceInput, {
      target: { value: "github.com/acme/payments" },
    });
    fireEvent.click(screen.getByRole("button", { name: "提交审核" }));

    await waitFor(() => expect(api.orgSubmitMemoryCandidate).toHaveBeenCalledWith(expect.objectContaining({
      targetScopeId: teamScope.id,
      kind: "howto",
      content: "发布前先验证回滚脚本",
      workspaceRef: "github.com/acme/payments",
      outcome: "恢复时间缩短",
    })));
    expect(screen.queryByPlaceholderText("写成可直接指导下一次任务的明确结论或步骤")).not.toBeInTheDocument();
  });
