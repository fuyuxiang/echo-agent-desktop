...
export function CodingEditor({
  ...
  const registerDiagnostics = (
    ...
    diagnosticsDisposableRef.current = monaco.editor.onDidChangeMarkers((resources) => {
      const model = editor.getModel();
      if (model && resources.some((resource) => resource.toString() === model.uri.toString())) publish();
    });
    publish();
  };
  ...
  const registerDiff: DiffOnMount = (editor, monaco) => {
    ...
    modified.addCommand(monaco.KeyMod.CtrlCmd | monaco.KeyCode.KeyS, () => saveHandlerRef.current());
  ...
  if (mode === "diff") {
    return (
      <div className="coding-monaco-editor" aria-label={`${path} 的差异`}>
        <DiffEditor
          key={`diff:${path}`}
          original={original}
          modified={value}
  ...
  return (
    <div className="coding-monaco-editor" aria-label={`编辑 ${path}`}>
      <Editor
        key={`edit:${path}`}
