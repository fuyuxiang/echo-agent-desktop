...
fn manual_install_cmd(channel: &str) -> String {
    ...
    if channel == "enterprise" {
        ...
        return if cfg!(windows) {
            "irm https://echo.agent/cli/enterprise-install.ps1 | iex".to_string()
        } else {
            "curl -fsSL https://echo.agent/cli/enterprise-install.sh | bash".to_string()
        };
    }
    if is_stable_channel(channel) || !safe {
        return if cfg!(windows) {
            "irm https://echo.agent/cli/install.ps1 | iex".to_string()
        } else {
            "curl -fsSL https://echo.agent/cli/install.sh | bash".to_string()
        };
    }
    if cfg!(windows) {
        format!("$env:GROK_CHANNEL='{channel}'; irm https://echo.agent/cli/install.ps1 | iex")
    } else {
        format!("curl -fsSL https://echo.agent/cli/install.sh | GROK_CHANNEL='{channel}' bash")
    }
}
