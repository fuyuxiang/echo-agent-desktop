...
const TOS_URL: &str = "https://echo.agent/legal/terms-of-service";
...
fn two_links_are_indexed_in_order() {
    let mut gate = gate();
    gate.body = Some("Read [Terms](https://echo.agent/a) and [Policy](https://echo.agent/b).".to_owned());
    ...
    assert_eq!(notice.links, vec!["https://echo.agent/a", "https://echo.agent/b"]);
}
...
fn a_url_we_will_not_open_degrades_to_plain_text() {
    for url in [
        ...
        "https://echo.agent/a&calc",
        // A percent pair is a variable to the same shell.
        "https://echo.agent/%USERNAME%",
    ] {
        let mut gate = gate();
...
fn a_link_with_nothing_to_paint_is_dropped() {
    let mut gate = gate();
    gate.body = Some("Read [ ](https://echo.agent/a) and [Terms](https://echo.agent/b).".to_owned());

    let notice = ConsentNotice::try_from_remote(&gate).expect("valid");

    assert_eq!(notice.links, vec!["https://echo.agent/b"]);
...
fn unpairable_markup_refuses() {
    for body in [
        ...
        "Read our [Terms](https://echo.agent/legal/te",
        // A url holds no space, so this `)` closes the second link, not the first.
        "Read [Terms](https://echo.agent/a [Policy](https://echo.agent/b) now.",
        ...
        "[a] b](https://echo.agent/legal/tos) applies.",
    ] {
        let mut gate = gate();
...
fn a_url_outside_a_link_refuses() {
    let mut gate = gate();
    gate.body = Some("Review the terms at https://echo.agent/legal/tos before continuing.".to_owned());
...
fn no_body_shape_leaks_a_url_or_raw_markup() {
    let bodies = [
        "[](https://echo.agent/legal/tos) applies.",
        "[note] see [AUP](https://echo.agent/legal/aup).",
        "Read [Terms](javascript:alert(1)) now.",
        "Read [Terms](https://echo.agent/a&calc) now.",
        // A `)` inside the url ends it early and leaves the rest to be painted as prose.
        "Read [Terms](https://echo.agent/a(b)https://evil.example) now.",
    ];
