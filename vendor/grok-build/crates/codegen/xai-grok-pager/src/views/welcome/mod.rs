...
mod tests {
    ...
    fn extract_user_code_parses_verification_url() {
        assert_eq!(
            extract_user_code("https://accounts.echo.agent/oauth2/device?user_code=ABCD-EFGH"),
            Some("ABCD-EFGH"),
        );
        // Trailing params after the code are ignored.
        assert_eq!(
            extract_user_code("https://echo.agent/oauth2/device?user_code=WXYZ-1234&foo=bar"),
            Some("WXYZ-1234"),
        );
        // A param whose name merely ends in `user_code` must not be matched.
        assert_eq!(
            extract_user_code("https://echo.agent/d?foo_user_code=BAD&user_code=GOOD"),
            Some("GOOD"),
        );
        // No code param, empty code, and unexpected characters all yield None.
        assert_eq!(extract_user_code("https://echo.agent/oauth2/device"), None);
        assert_eq!(extract_user_code("https://echo.agent/d?user_code="), None);
        assert_eq!(extract_user_code("https://echo.agent/d?user_code=AB%20CD"), None);
    }
    ...
    fn device_auth_arm_shows_url_and_no_paste_box() {
        ...
        let mut buf = Buffer::empty(area);
        let theme = Theme::current();
        let url = "https://accounts.echo.agent/oauth2/device?user_code=ABCD-EFGH";
    ...
    fn device_auth_arm_raw_url_mode_shows_full_url() {
        ...
        let mut buf = Buffer::empty(area);
        let theme = Theme::current();
        let url = "https://accounts.echo.agent/oauth2/device?user_code=WXYZ-1234";
    ...
    fn raw_url_mode_uses_full_width_for_long_urls() {
        ...
        let url = "https://accounts.echo.agent/oauth2/device?user_code=WXYZ-1234&extra=0123456789";
    ...
    fn command_auth_arm_shows_url_and_waiting() {
        ...
        let mut buf = Buffer::empty(area);
        let theme = Theme::current();
        let url = "https://accounts.echo.agent/oauth2/authorize?client_id=grok";
