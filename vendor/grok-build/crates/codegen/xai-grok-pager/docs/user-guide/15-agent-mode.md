...
Beyond the base ACP protocol, Grok defines extension methods under the `echo.agent/` prefix for SpaceXAI-specific functionality. These cover:
...
The tables here show representative methods in each category. The `echo.agent/*` set is SpaceXAI-specific and may expand across releases, so treat it as non-exhaustive and discover the available methods from the agent's `initialize` response.
