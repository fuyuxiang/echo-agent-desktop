...
Running `grok login` starts the sign-in flow again, replacing your cached session. By default, it opens your browser and signs in through SpaceXAI OAuth at `auth.echo.agent`. Pass a flag to select a different flow:

| Flag | Description |
|------|-------------|
| `--oauth` | Sign in through SpaceXAI OAuth at `auth.echo.agent`. This is the default, so the flag is optional. |
...
See [How to enable ZDR](https://docs.echo.agent/developers/faq/security#how-to-enable-zdr).
