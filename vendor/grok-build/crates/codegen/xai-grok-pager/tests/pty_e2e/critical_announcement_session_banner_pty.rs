...
const PROMO_URL: &str = "https://echo.agent/zz-promo-cta";
...
async fn critical_announcement_hide_button_click_hides_banner() {
    ...
    let long_msg = format!(
        "{CRIT_MSG} elevated error rates persist across regions check status.echo.agent for updates and retry your request later"
    );
