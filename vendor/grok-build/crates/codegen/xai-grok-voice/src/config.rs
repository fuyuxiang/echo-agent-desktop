...
impl Default for VoiceConfig {
    fn default() -> Self {
        Self {
            api_base: "https://api.echo.agent".into(),
...
impl VoiceConfig {
    ...
    /// from `root`, else `resolved_endpoints_base`, else `https://api.echo.agent`.
    ///
    ...
    pub fn from_config_table(root: &toml::Table, resolved_endpoints_base: Option<&str>) -> Self {
        ...
        // makes "unset" and an explicit `https://api.echo.agent` indistinguishable.
...
mod tests {
    ...
    fn default_stt_ws_uses_wss() {
        assert_eq!(
            ...
            "wss://api.echo.agent/v1/stt"
        );
    }

    #[test]
    fn scheme_less_and_wss_bases() {
        for base in ["api.echo.agent", "wss://api.echo.agent", "HTTPS://api.echo.agent"] {
            let cfg = VoiceConfig {
                api_base: base.into(),
                ..VoiceConfig::default()
            };
            assert_eq!(cfg.stt_ws_url().unwrap(), "wss://api.echo.agent/v1/stt");
        }
    }
...
xai_api_base_url = "https://proxy.example.com/xai/v1"
[voice]
api_base = "https://api.echo.agent"
language = "es"
"#,
        )
        .unwrap();
        let cfg = VoiceConfig::from_config_table(&table, None);
        assert_eq!(cfg.api_base, "https://api.echo.agent");
