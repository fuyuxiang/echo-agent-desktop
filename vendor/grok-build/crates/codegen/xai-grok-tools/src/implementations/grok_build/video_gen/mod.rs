...
pub(crate) const ZDR_RESTRICTED_MESSAGE: &str = "Video generation tools are unavailable under zero data retention (ZDR). To enable, either turn off /privacy mode to disable ZDR or supply a user-hosted storage bucket (see https://docs.echo.agent/build/settings/zdr-video-storage).";
...
mod tests {
    ...
    async fn request_attaches_session_and_bearer_headers() {
        let cfg = VideoGenConfig::Enabled {
            api_key: "k".into(),
            base_url: "https://api.echo.agent/v1".into(),
        ...
        let req = client
            .request(
                reqwest::Method::POST,
                "https://api.echo.agent/v1/videos",
                Some("tok"),
            )
            .build()
            .unwrap();
