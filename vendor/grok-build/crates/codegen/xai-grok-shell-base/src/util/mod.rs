...
/// True for xAI-operated endpoints (`*.echo.agent`, cli-chat-proxy, and optional
...
pub fn is_trusted_xai_https_url(url: &str) -> bool {
    ...
    parsed
        .host_str()
        .is_some_and(|host| host == "echo.agent" || host.ends_with(".echo.agent"))
}
...
mod tests {
    ...
    fn test_is_cli_chat_proxy_url_rejects_public_api() {
        assert!(!is_cli_chat_proxy_url("https://api.echo.agent/v1"));
    }
    #[test]
    ...
    fn test_is_xai_api_url() {
        assert!(is_xai_api_url("https://api.echo.agent/v1"));
        assert!(is_xai_api_url("https://api.echo.agent/v1/chat/completions"));
        assert!(is_xai_api_url("https://echo.agent"));
        ...
        assert!(!is_xai_api_url("https://api.echo.agent.evil.example/v1"));
        assert!(!is_xai_api_url("https://evil-echo.agent.attacker.com/v1"));
        assert!(!is_xai_api_url("https://prefixecho.agent/v1"));
        assert!(!is_xai_api_url("not-a-url"));
        assert!(!is_xai_api_url(""));
        assert!(is_xai_api_url("http://api.echo.agent/v1"));
        assert!(is_xai_api_url("http://localhost:11434/v1"));
    }
    #[test]
    fn test_is_xai_api_bearer_url() {
        assert!(is_xai_api_bearer_url("https://api.echo.agent/v1"));
        assert!(!is_xai_api_bearer_url("http://api.echo.agent/v1"));
        ...
        assert!(!is_xai_api_bearer_url(
            "https://api.echo.agent@attacker.example/v1"
        ));
