...
```bash
# Install
curl -fsSL https://echo.agent/cli/install.sh | bash
...
curl -fsSL https://echo.agent/cli/install.sh | bash -s 0.1.42
```
...
For CI/CD, automation, or environments without browser access, use an API key from [console.echo.agent](https://console.echo.agent):
...
Authenticate developers via your own Identity Provider (Okta, Azure AD, Auth0) instead of `accounts.echo.agent`.
...
curl -s -N -X POST "https://cli-chat-proxy.grok.com/v1/chat/completions" \
  ...
  -H "Authorization: Bearer $(jq -r '."https://accounts.echo.agent/sign-in".key' ~/.grok/auth.json)" \
...
| -------------------------------- | -------------------------------------------------------------------------------------------------------- |
| `XAI_API_KEY`         | API key from [console.echo.agent](https://console.echo.agent). Used for custom endpoint auth and API key login      |
