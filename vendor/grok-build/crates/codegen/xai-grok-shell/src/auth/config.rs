...
pub const XAI_OAUTH2_ISSUER: &str = "https://auth.echo.agent";
...
const PROD_ACCOUNTS_APP_ORIGINS: &[&str] = &["https://accounts.echo.agent"];
...
/// Matches the key format produced by the original `accounts.echo.agent` relay auth.
pub(crate) const LEGACY_AUTH_SCOPE: &str = "https://accounts.echo.agent/sign-in";
impl GrokComConfig {
...
mod tests {
    ...
    fn team_auth_scope_is_base_scope() {
        let cfg = OAuth2ProviderConfig {
            issuer: "https://auth.echo.agent".into(),
        ...
        assert_eq!(cfg.auth_scope(), "https://auth.echo.agent::client-123");
    }
    #[test]
    ...
    /// (served from accounts.echo.agent) delivers the code via `fetch(..., cors)`, so
    ...
    fn allowed_accounts_app_origins_are_frozen() {
        assert_eq!(PROD_ACCOUNTS_APP_ORIGINS, &["https://accounts.echo.agent"]);
