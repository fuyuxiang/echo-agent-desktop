...
fn inject_yolo_notification_adds_client_identifier() {
    let mut json =
        pv(r#"{"jsonrpc":"2.0","method":"echo.agent/yolo_mode_changed","params":{"yolo_mode":true}}"#);
...
async fn models_update_broadcasts_to_all_clients() {
    ...
    let update = r#"{"jsonrpc":"2.0","method":"_echo.agent/models/update","params":{"currentModelId":"grok-new","availableModels":[{"modelId":"grok-new","name":"Grok New"}]}}"#;
...
async fn mcp_servers_updated_broadcasts_to_all_clients() {
    ...
    let update = r#"{"jsonrpc":"2.0","method":"_echo.agent/mcp/servers_updated","params":{"method":"echo.agent/mcp/servers_updated","params":{"mcpServers":[{"name":"grok_com_slack","source":"managed"}]}}}"#;
    response_tx.send(update.to_string()).unwrap();
