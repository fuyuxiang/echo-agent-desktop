...
fn prepare_replay_cursor_refused_when_tail_has_event_id_less_line() {
    ...
    let old_xai = r#"{"timestamp":2,"method":"_echo.agent/session/update","params":{"sessionId":"s","update":{"sessionUpdate":"hook_annotation","message":"trailing"}}}"#;
    let raw = format!("{a1}\n{old_xai}\n");
    ...
    let new_xai = r#"{"timestamp":2,"method":"_echo.agent/session/update","params":{"sessionId":"s","update":{"sessionUpdate":"hook_annotation","message":"trailing"},"_meta":{"eventId":"ev2"}}}"#;
    let raw = format!("{a1}\n{new_xai}\n");
    let prepared = prepare_replay_lines(&raw, Some("ev1"));
    assert!(!prepared.mark_replay);
    assert_eq!(prepared.lines.len(), 1);
