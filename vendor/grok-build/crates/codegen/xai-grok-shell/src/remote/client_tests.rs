...
fn parse_openai_format_uses_id_field() {
    ...
    let result = parse_remote_model_value(&value, "https://api.echo.agent/v1").unwrap();
    assert_eq!(result.model, "grok-3");
    assert_eq!(result.base_url, "https://api.echo.agent/v1");
...
fn list_url_derived_from_base_url() {
    ...
    assert_eq!(ep.resolve_models_list_url(), "https://api.echo.agent/v1/models");
}
#[test]
...
fn models_fetch_endpoint_matches_auth_mode() {
    ...
    assert_eq!(
        ...
        "https://api.echo.agent/v1/models"
    );
