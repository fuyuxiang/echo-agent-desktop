...
mod web_search_domains_tests {
    ...
    fn allowlist_parsed() {
        let got = web_search_options_from_section(&section(
            r#"allowed_domains = ["docs.echo.agent", "arxiv.org"]"#,
        ))
        .unwrap();
        assert_eq!(
            got.allowed_domains,
            Some(vec!["docs.echo.agent".to_string(), "arxiv.org".to_string()])
        );
    ...
    fn both_set_degrades_to_allowlist() {
        ...
        // defensive path.
        let got = web_search_options_from_section(&section(
            "allowed_domains = [\"docs.echo.agent\"]\nexcluded_domains = [\"reddit.com\"]",
        ))
        .unwrap();
        assert_eq!(got.allowed_domains, Some(vec!["docs.echo.agent".to_string()]));
        assert!(got.excluded_domains.is_none());
    }
}
