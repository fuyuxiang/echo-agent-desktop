...
async fn test_set_model_broadcasts_to_session_subscribers() {
    ...
    let broadcast = format!(
        r#"{{"jsonrpc":"2.0","method":"echo.agent/session_notification","params":{{"sessionId":"{}","update":{{"sessionUpdate":"model_changed","model_id":"grok-4","reasoning_effort":"high"}}}}}}"#,
        shared_sid
    );
...
async fn test_session_ownership_cleanup_on_disconnect() {
    ...
    assert_eq!(eviction_json["method"], "_echo.agent/internal/evict_sessions");
