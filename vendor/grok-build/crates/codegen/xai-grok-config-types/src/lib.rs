...
pub struct RemoteSettings {
    ...
    /// OAuth2 provider issuer URL (e.g., "https://auth.echo.agent"). When present
    ...
    /// When `Some(true)`, enable grok's default OAuth2 (xAI auth.echo.agent).
    ...
    fn remote_settings_consent_gate_round_trip() {
        let json = r#"{
            "consent_gate": {
                ...
                "version": 3,
                "title": "Updated terms",
                "body": "Review our [Terms of Service](https://echo.agent/legal/tos) before continuing.",
        ...
        assert!(
            gate.body
                .as_deref()
                .is_some_and(|b| b.contains("https://echo.agent/legal/tos"))
        );
    }
