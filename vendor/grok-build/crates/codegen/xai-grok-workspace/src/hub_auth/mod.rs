...
mod tests {
    ...
    fn read_auth_entry_tolerates_extra_fields() {
        ...
        let path = write_auth_json(
            ...
            "scope": {
                ...
                "first_name": "Test",
                "refresh_token": "rt1",
                "oidc_issuer": "https://auth.echo.agent",
                "oidc_client_id": "c1",
    ...
    fn build_oidc_provider_requires_refresh_token() {
        let entry = AuthEntry {
            ...
            user_id: "u1".into(),
            refresh_token: None,
            oidc_issuer: Some("https://auth.echo.agent".into()),
    ...
    fn write_refreshed_token_updates_jwt_entry() {
        ...
        let path = write_auth_json(
            ...
            "oidc": { "key": "eyJ.old", "user_id": "u2", "refresh_token": "rt-old", "oidc_issuer": "https://auth.echo.agent" }
        }"#,
        );
    ...
    fn read_auth_entry_prefers_latest_expiry() {
        ...
        let path = write_auth_json(
            dir.path(),
            r#"{
            "aaa-stale": { "key": "eyJ.a", "refresh_token": "rt-a", "oidc_issuer": "https://auth.echo.agent", "expires_at": "2026-01-01T00:00:00Z" },
            "zzz-active": { "key": "eyJ.z", "refresh_token": "rt-z", "oidc_issuer": "https://auth.echo.agent", "expires_at": "2026-06-01T00:00:00Z" }
        }"#,
        );
        ...
        let path = write_auth_json(
            dir.path(),
            r#"{
            "aaa-with-expiry": { "key": "eyJ.a", "refresh_token": "rt-a", "oidc_issuer": "https://auth.echo.agent", "expires_at": "2026-01-01T00:00:00Z" },
            "zzz-no-expiry": { "key": "eyJ.z", "refresh_token": "rt-z", "oidc_issuer": "https://auth.echo.agent" }
        }"#,
        );
    ...
    fn write_refreshed_token_targets_exact_scope_key() {
        ...
        let path = write_auth_json(
            dir.path(),
            r#"{
            "zzz": { "key": "eyJ.z", "refresh_token": "rt-z", "oidc_issuer": "https://auth.echo.agent" },
            "aaa": { "key": "eyJ.a", "refresh_token": "rt-a", "oidc_issuer": "https://auth.echo.agent" }
        }"#,
        );
    ...
    fn write_refreshed_token_does_not_roll_back_a_newer_token() {
        ...
        let path = write_auth_json(
            dir.path(),
            r#"{
            "oidc": { "key": "eyJ.newer", "refresh_token": "rt-newer", "oidc_issuer": "https://auth.echo.agent", "expires_at": "2026-06-01T00:00:00Z" }
        }"#,
        );
    ...
    fn write_refreshed_token_preserves_existing_rt_when_not_rotated() {
        ...
        let path = write_auth_json(
            dir.path(),
            r#"{
            "oidc": { "key": "eyJ.old", "user_id": "u1", "refresh_token": "rt-keep", "oidc_issuer": "https://auth.echo.agent" }
        }"#,
        );
    ...
    fn provider_loopback_uses_bearer() {
        ...
        let path = write_auth_json(
            dir.path(),
            r#"{ "oidc": { "key": "eyJ.tok", "user_id": "u1", "refresh_token": "rt", "oidc_issuer": "https://auth.echo.agent", "oidc_client_id": "c1" } }"#,
        );
