//! Process-isolated: valid ECHO_AGENT_EXTRA_CA_BUNDLE loads one root via OnceLock.
...
-----END CERTIFICATE-----\n";
    ...
    unsafe {
        std::env::set_var(
            echo_agent_extra_ca::ENV_ECHO_AGENT_EXTRA_CA_BUNDLE,
            path.as_os_str(),
        );
    }

    assert_eq!(echo_agent_extra_ca::extra_root_ders().len(), 1);
