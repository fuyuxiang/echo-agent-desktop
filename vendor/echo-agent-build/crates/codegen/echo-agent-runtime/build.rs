//! Build script for bundling ripgrep for the echo-agent-shell crate.
//!
//! - If `ECHO_AGENT_SHELL_BUNDLE_RG_PATH` is set, always bundle it
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("cargo:rerun-if-env-changed=ECHO_AGENT_SHELL_BUNDLE_RG_PATH");
    println!("cargo:rerun-if-env-changed=ECHO_AGENT_SHELL_RG_DOWNLOAD_BASE");
    ...
    let path_override = env::var("ECHO_AGENT_SHELL_BUNDLE_RG_PATH").ok();
    ...
    // ECHO_AGENT_ROOT/target/tmp (which is read-only inside the sandbox). Outside
    // Bazel, prefer ECHO_AGENT_ROOT's shared cache dir (monorepo behavior) and fall
    // back to OUT_DIR for standalone checkouts where ECHO_AGENT_ROOT is not a thing.
    ...
    } else if let Ok(echo_agent_root) = env::var("ECHO_AGENT_ROOT") {
        PathBuf::from(echo_agent_root).join("target/tmp/echo-agent-shell-bundle-rg")
    } else {
        PathBuf::from(env::var("OUT_DIR")?)
    };
    ...
    // An explicit ECHO_AGENT_SHELL_BUNDLE_RG_PATH still bundles on Windows (the
    ...
    println!("cargo:rustc-env=ECHO_AGENT_SHELL_RG_VER={}", RG_VER);
    println!(
        "cargo:rustc-env=ECHO_AGENT_SHELL_RG_GEN_DIR={}",
        gen_dir.display()
    );
    ...
    if let Some(path) = path_override {
        ...
        println!("cargo:rustc-env=ECHO_AGENT_SHELL_RG_TARGET=override");
        let _ = fs::remove_file(&dest);
        fs::copy(PathBuf::from(path.clone()), &dest).map_err(|e| {
            format!(
                "Failed copying ECHO_AGENT_SHELL_BUNDLE_RG_PATH: {e} from path {path} to dest {}",
                dest.display()
            )
        })?;
        return Ok(());
    ...
    let asset_triple = match (target_os.as_str(), target_arch.as_str()) {
        ...
        _ => {
            return Err(format!(
                "Unsupported target for ripgrep bundling: {os}-{arch}. Set ECHO_AGENT_SHELL_BUNDLE_RG_PATH to a local rg binary for offline or unsupported builds.",
                os = target_os,
                arch = target_arch
            ).into());
        }
    ...
    println!(
        "cargo:rustc-env=ECHO_AGENT_SHELL_RG_TARGET={}",
        asset_triple
    );
    ...
    // internal mirror (e.g. ECHO_AGENT_SHELL_RG_DOWNLOAD_BASE=http://<mirror>/github/
    // BurntSushi/ripgrep/releases/download). Defaults to the public GitHub
    // releases URL.
    let download_base = env::var("ECHO_AGENT_SHELL_RG_DOWNLOAD_BASE")
    ...
    let bytes: Vec<u8> = {
        let resp = reqwest::blocking::get(&url).map_err(|e| {
            format!(
                "Failed to download ripgrep: {}\nSet ECHO_AGENT_SHELL_BUNDLE_RG_PATH to a local rg for offline builds.",
                e
            )
        })?;
        if !resp.status().is_success() {
            return Err(format!(
                "HTTP {} downloading ripgrep. Set ECHO_AGENT_SHELL_BUNDLE_RG_PATH for offline builds.",
                resp.status()
            )
            .into());
        }
    ...
    if !found {
        return Err(format!(
            "Could not find 'rg' in ripgrep archive {}. Set ECHO_AGENT_SHELL_BUNDLE_RG_PATH for offline builds.",
            url
        )
        .into());
    }
