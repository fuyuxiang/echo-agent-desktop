#!/bin/bash
#
# EchoAgent CLI installer (enterprise channel) — https://echo-agent.invalid/cli/enterprise-install.sh
#
...
# Env: ECHO_AGENT_BIN_DIR, ECHO_AGENT_PROXY_URL
#
# Usage:
#   curl -fsSL https://echo-agent.invalid/cli/enterprise-install.sh | bash            # latest enterprise
#   curl -fsSL https://echo-agent.invalid/cli/enterprise-install.sh | bash -s 0.1.42  # specific version
#   ECHO_AGENT_DEPLOYMENT_KEY=<key> bash <(curl -fsSL https://echo-agent.invalid/cli/enterprise-install.sh)
#
...
if [ "$os" = "windows" ]; then
    ...
    chmod +x "$binary_path"
    ln -sf "$binary_path" "$BIN_DIR/echoagent"
