...
impl ModelState {
    ...
    /// Machine-readable model ID string for the current model (e.g. "echo-agent-4.5").
    ...
    /// correct today (all current EchoAgent models accept images, so nothing is
    ...
    /// not offer (e.g. `none` on echo-agent-4.5) are rejected so we fail in the TUI
