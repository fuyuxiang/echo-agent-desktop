...
pub(crate) fn is_server_initiated_prompt(prompt_id: &str) -> bool {
    echo_agent_runtime::session::PromptOrigin::from_prompt_id(prompt_id).is_synthetic()
}
...
/// `echo.agent/session/prompt_complete` turn-end signal. A viewer can thus safely
...
pub(crate) fn is_scheduler_fired_prompt(prompt_id: &str) -> bool {
    matches!(
        echo_agent_runtime::session::PromptOrigin::from_prompt_id(prompt_id),
        echo_agent_runtime::session::PromptOrigin::SchedulerFired
    )
}
...
pub(crate) fn is_wake_prompt(prompt_id: &str) -> bool {
    matches!(
        echo_agent_runtime::session::PromptOrigin::from_prompt_id(prompt_id),
        echo_agent_runtime::session::PromptOrigin::TaskCompleted { .. }
            | echo_agent_runtime::session::PromptOrigin::SubagentCompleted { .. }
            | echo_agent_runtime::session::PromptOrigin::WorkflowCompleted { .. }
            | echo_agent_runtime::session::PromptOrigin::NotificationDrain
    )
}
...
/// adoptable iff the turn emits a terminal `echo.agent/session/prompt_complete`, the
