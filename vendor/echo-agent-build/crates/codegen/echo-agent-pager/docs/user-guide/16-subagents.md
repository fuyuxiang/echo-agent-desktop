...
| **How you set them** | At startup, or with agent definitions (`.md` files in `.echo-agent/agents/` or `~/.echo-agent/agents/`) | In `config.toml` (`[subagents.personas]`) or `.toml` files under `.echo-agent/personas/`; applied during subagent resolution |
...
| **Examples** | `echo-agent-build`, `explore`, `plan` | `researcher`, `concise` |
...
EchoAgent Build discovers file-based personas from these locations, in priority order:

- `.echo-agent/personas/*.toml` (project)
- `~/.echo-agent/personas/*.toml` (user)
...
> **Note:** EchoAgent Build applies personas through subagent resolution and roles, not through a `spawn_subagent` parameter. The main agent does not pass a persona name when it spawns a child.
...
When a persona applies, EchoAgent Build resolves the effective model and reasoning effort in this order, highest priority first:
...
EchoAgent Build manages worktrees through the `echo.agent/git/worktree/*` extension methods, including an apply operation that merges changes back into the main working directory.
...
[subagents.models]
explore = "echo-agent-build"               # route explore to a specific model
```
...
# instructions_file = ".echo-agent/personas/concise.md"  # or load from a file
```

EchoAgent Build also discovers roles from `.echo-agent/roles/*.toml` and personas from `.echo-agent/personas/*.toml`. Inline `config.toml` definitions take precedence over files.

---

## The Tasks Pane (TUI)

EchoAgent Build shows running and finished work in side panes on the agent screen:
...
- `Subagent running: "do the thing" (Implementer · echo-agent-3) · Thinking`
