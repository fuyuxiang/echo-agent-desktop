...
/// The base prompt template source, exposed for `echoagent prompt --section template`.
...
/// Apply-patch prompt template source, exposed for `echoagent prompt --section apply-patch-template`.
...
mod tests {
    ...
    /// Build a TemplateRenderer with the standard echo-agent-build tool kinds.
    ...
    fn test_base_template_renders() {
        ...
        assert!(!prompt.contains("released by EchoAgent"));
