...
impl WebSearchClient {
    ...
    pub async fn search(
        ...
        query: &str,
        allowed_domains: Option<Vec<String>>,
    ) -> Result<(String, Vec<String>), echo_agent_tool_runtime::ToolError> {
        ...
        if status == reqwest::StatusCode::UNAUTHORIZED {
            ...
            return Err(echo_agent_tool_runtime::ToolError::unauthorized(format!(
    ...
    pub async fn search_with_titles(
        ...
        query: &str,
        allowed_domains: Option<Vec<String>>,
    ) -> Result<(String, Vec<(String, String)>), echo_agent_tool_runtime::ToolError> {
        let (allowed, excluded) = self.resolve_filters(allowed_domains);
        let request = self.build_request_json(query, allowed, excluded)?;
        let url = format!("{}/responses", self.base_url.trim_end_matches('/'));
        let sent_bearer = self.current_bearer().await;
...
mod tests {
    ...
    fn build_request_json_allowlist_only_has_no_excluded_key() {
        let client = client_with_defaults(None, None);
        let body = client
            .build_request_json("q", Some(v(&["docs.echo-agent.invalid"])), None)
            .expect("request json builds");
        let filters = &body["tools"][0]["filters"];
        assert_eq!(
            filters["allowed_domains"],
            serde_json::json!(["docs.echo-agent.invalid"])
        );
