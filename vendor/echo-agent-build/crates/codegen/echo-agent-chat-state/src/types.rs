...
/// emitter in `echo-agent-runtime` and the upsert/detection here — a drift would
...
pub enum AuthType {
    /// From AuthManager (echoagent login, OIDC, external binary). Refreshable.
    #[default]
    SessionToken,
    /// From user config ([model.*] api_key, env_key, ECHO_AGENT_API_KEY). Not refreshable.
    ApiKey,
}
...
/// `echo_agent_sampling_types::SamplingConfig` (which is secret-free).
