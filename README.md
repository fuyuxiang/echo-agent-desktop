...
- 内置对话模型连接 `http://123.56.188.16:8088/v1`，提供 `chat-xc`、`chat-glm`、`chat-qwen`，默认使用 `chat-xc`；登录组织后优先使用组织下发模型，手动指定的默认模型始终优先。记忆默认开启并使用本机全文检索，不默认请求远端向量化或重排。摘要和整理仍使用当前会话的模型。显式配置的自定义检索服务会保留；设置中手动选择并确认“内置远端服务”后，才使用 `embed-pro` / `rerank-pro`。内置服务当前使用明文 HTTP，处理敏感内容请选用可信的 HTTPS 模型连接和本机检索。检索方式修改在重启 Agent 后生效。
- 个人模型连接默认使用 HTTPS。需要接入仅提供 HTTP 的服务时，在连接编辑器中为该连接单独勾选「允许此个人连接使用 HTTP」；API Key、提问和回复会明文传输。此选项不改变 HTTPS 连接或组织下发模型的连接策略。

- 草稿、附件引用和消息队列按版本保存在本机；重启后队列暂停恢复，在途消息标记为“发送结果待确认”，不会自动重发。存储写入失败会显示提示并阻止新消息入队；损坏的数据会保留原始内容。
- 「设置 → 数据管理」可导出带版本和逐文件 SHA-256 校验的 ZIP 备份，涵盖会话、项目资料、代码任务、记忆、会议、应用管理的粘贴附件及界面状态；不包含模型配置、组织凭据、外部工作目录原文件。恢复前预览范围，重启前停止 Runtime 和 IDE，恢复失败回滚；恢复的定时任务、代码执行和消息队列保持暂停。原有被替换文件保留在 `restore-previous-*`，确认恢复结果后可自行清理。
- 项目中的旧计划会迁移为工作项，原始计划保留在 `legacyPlans`。列表和看板是同一组数据；“本轮结束”表示 Agent 已停止输出，“已验收”由用户检查成果后确认。
