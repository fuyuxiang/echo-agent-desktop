...
- 内置对话模型连接 `http://www.ojlab.com:8088/v1`，提供 `chat-xc`、`chat-glm`、`chat-qwen`，默认使用 `chat-xc`；登录组织后优先使用组织下发模型，手动指定的默认模型始终优先。记忆默认开启，向量化使用 `embed-pro`（1024 维），重排使用 `rerank-pro`，均无需 API Key。该服务当前使用明文 HTTP；处理敏感内容前请审查相关配置，或在「设置 → 记忆」中关闭记忆。
